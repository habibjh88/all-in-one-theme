...
class PostMetaController {
	...
	function add_meta_box(): void {
		...
		$this->postmeta->add_meta_box( "rt_el_builder_settings", __( 'Header - Footer Builder Settings', 'newsfit-core' ), [ 'elementor-newsfit' ], '', '', 'high',
			[
				'fields' => [
					"rt_el_builder_meta" => [
						'label' => __( 'Header - Footer Builder Settings', 'newsfit-core' ),
						'type'  => 'group',
						'value' => $this->get_el_builder_meta_args(),
					],
				],
			]
		);
	...
	function get_el_builder_meta_args() {
		...
		return apply_filters( 'newsfit_layout_meta_field', [
			...
			'show_on' => [
				...
				'type'    => 'multi_select2',
				'options' => Builder::get_builder_type(),
				'default' => 'default',
			],
		] );
	}
	...
	function get_post_page_meta_args() {
		...
		return apply_filters( 'newsfit_layout_meta_field', [
			...
			'header_style'      => [
				'label'   => __( 'Header Layout', 'newsfit-core' ),
				'type'    => 'select',
				'options' => [
					'default' => __( 'Default from customizer', 'newsfit-core' ),
